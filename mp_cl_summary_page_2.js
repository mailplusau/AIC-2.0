        ...
        function onclickContinueReview(internalID, locked, sc_ID) {
            ...
            if (valueMonth == false) {
                ...
                if (isNullorEmpty(internalID)) {
                    ...
                    searchedJobsExtras.filters.push(search.createFilter({ name: 'custrecord_job_franchisee', join: null, operator: search.Operator.ANYOF, values: customScript.getValue({ fieldId: 'zee' }) }));
                    if (!isNullorEmpty(customScript.getValue({ fieldId: 'start_date' })) && !isNullorEmpty(customScript.getValue({ fieldId: 'end_date' }))) {
                        searchedJobsExtras.filters.push(search.createFilter({ name: 'custrecord_job_date_scheduled', join: null, operator: search.Operator.ONORAFTER, values: customScript.getValue({ fieldId: 'start_date' }) }));
                        searchedJobsExtras.filters.push(search.createFilter({ name: 'custrecord_job_date_scheduled', join: null, operator: search.Operator.ONORBEFORE, values: customScript.getValue({ fieldId: 'end_date' }) }));
                    }
